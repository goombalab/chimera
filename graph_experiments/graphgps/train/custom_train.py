...
def custom_train(loggers, loaders, model, optimizer, scheduler):
    ...
    if cfg.wandb.use and dist.get_rank() == 0:
        try:
            import wandb
        except:
    ...
    num_splits = len(loggers) if loggers is not None else 3
    ...
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        ...
        # Train epoch on distributed train_loader (train_loader[0])
        loaders[0].sampler.set_epoch(cur_epoch)
        train_epoch(
            loggers[0] if dist.get_rank() == 0 else None,
            loaders[0],
            model,
            optimizer,
            scheduler,
        ...
        # Append training performance only on rank 0
        if dist.get_rank() == 0:
        ...
        scheduler.step()  # Regular stepping for other schedulers
        ...
        if (
            ...
            and dist.get_rank() == 0  # Only rank 0 saves checkpoints
        ):
        ...
        if is_eval_epoch(cur_epoch) and dist.get_rank() == 0:
            val_perf = perf[1]
